...
if __name__ == "__main__":
    ref = cv2.imread(
        "/efs/datasets/floater_dataset_edited/Tracker_sample/clean_non_reg/322.jpg", 0
    )
    trg = cv2.imread(
        "/efs/datasets/floater_dataset_edited/Tracker_sample/clean_non_reg/328.jpg", 0
    )

    # ref, blurred_area = apply_blur(ref, 600, (511, 255))
    # print(blurred_area)

    apply_landmark = LandMarkRegistration(ref, method="surf")

    targ_transferred = apply_landmark(trg)
    error = apply_landmark.evaluate_reg()
    print("Error: ", error)
    # targ_transferred = cv2.resize(targ_transferred, (512, 512))
    img_3d = np.dstack([ref, np.zeros_like(ref), targ_transferred])

    cv2.imwrite(
        "/home/ubuntu/Projects/Floater_tracking/.samples/landmark_reg/reg.png", img_3d
    )
    cv2.imwrite(
        "/home/ubuntu/Projects/Floater_tracking/.samples/landmark_reg/ref.png", ref
    )
    cv2.imwrite(
        "/home/ubuntu/Projects/Floater_tracking/.samples/landmark_reg/targ_transferred.png",
        targ_transferred,
    )
